...
SELECT   ps.PatientId,
         ps.Hospital,
         CASE ps.Hospital WHEN 'PRUH' THEN 'Trust A' WHEN 'Oxleas' THEN 'Trust A' ELSE 'Trust B' END AS Trust,
         ps.Ward
...
SELECT   ps.PatientId,
         ...
         ps.Ward,
         ps.Tariff,
         CASE WHEN ps.Tariff > 8 THEN 'ICU' WHEN ps.Ward LIKE '%Surgery' THEN 'Surgical' WHEN ps.Ward IN ('Ophthalmology', 'Emergency', 'Emergency Surgery') THEN 'A&E' ELSE 'General' END AS WardType
FROM     dbo.PatientStay AS ps
ORDER BY WardType;
...
*/
SELECT   SMY.FinancialYear,
         count(*) AS NumberOfAdmissions
FROM     (SELECT ps.PatientId,
                 ps.AdmittedDate,
                 CASE WHEN DATEPART(MONTH, ps.AdmittedDate) >= 3 THEN CONCAT('FY-', DATEPART(YEAR, ps.AdmittedDate), '-', DATEPART(YEAR, ps.AdmittedDate) + 1) ELSE CONCAT('FY-', DATEPART(YEAR, ps.AdmittedDate) - 1, '-', DATEPART(YEAR, ps.AdmittedDate)) END AS FinancialYear
          FROM   dbo.PatientStay AS ps
          WHERE  ps.Hospital = 'PRUH') AS smy

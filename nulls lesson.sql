...
SELECT COUNT(m.Colour) AS NonNullColourColumnCount
FROM   Message AS m;
...
SELECT m.MessageId,
       ...
       COALESCE(m.Colour, 'No Colour') AS FullColour1
FROM   Message AS m;
...
SELECT m.MessageId,
       ...
       isnull(ISNULL (m.Colour, m.Style), 'Nothing to see here') AS cOMBINEDISNULL
FROM   Message AS m;
...
SELECT m.MessageId,
       ...
       m.Style,
       CASE 
       WHEN m.Colour IS NULL AND m.Style IS NULL THEN 1 
       ELSE 0 
       END AS IsColourAndStyleMissing
FROM   Message AS m;

-- Summarise for the whole table
SELECT COUNT(*) AS NumMessages,
       SUM(CASE WHEN m.Colour IS NULL
                     AND m.Style IS NULL THEN 1 ELSE 0 END) AS NumMessagesWithColourAndStyleMissing
FROM   Message AS m;

-- Summarise for each region
SELECT   m.Region,
         COUNT(*) AS NumMessages,
         COUNT(*) - SUM(CASE WHEN m.Colour IS  NULL
                       AND m.Style IS  NULL THEN 1 ELSE 0 END) AS vALIDmESSAGES
FROM     Message AS m
GROUP BY m.Region;
...
SELECT   ps.PatientId,
         ps.Hospital,
         CASE ps.Hospital WHEN 'Kingston' THEN 'Inner' WHEN 'Kings College' THEN 'Inner' ELSE 'Outer' END AS HospitalLocation
...
SELECT   ps.PatientId,
         ...
         ps.Ward,
         ps.Ethnicity,
         CASE WHEN ps.Ethnicity IS NULL THEN 'No' ELSE 'Yes' END AS DetailsKnown
FROM     dbo.PatientStay AS ps
ORDER BY ps.PatientId;
...
SELECT   ps.PatientId,
         ps.AdmittedDate,
         ps.Tariff,
         CASE WHEN ps.Tariff >= 7 THEN 'high' WHEN ps.Tariff >= 4 THEN 'medium' ELSE 'low' END AS PatientTariffGroup
FROM     dbo.PatientStay AS ps
ORDER BY ps.PatientId;
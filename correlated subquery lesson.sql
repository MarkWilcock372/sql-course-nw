...
SELECT
	...
	,ps.PatientId
	,ps.Tariff
	, (SELECT SUM(Tariff) FROM PatientStay WHERE Hospital = ps.Hospital) AS TotalHospitalTariff
...
SELECT
    ...
    ,  ((SELECT MIN(PatientId) FROM PatientStay WHERE PatientId > ps.PatientId) - (	SELECT MAX(PatientId) FROM PatientStay WHERE PatientId < ps.PatientId)) /2 AS Gap
FROM
	PatientStay ps
ORDER BY
	Gap DESC
...
SELECT 
    abc.PatientId,
    abc.PreviouslyBookedPatientId,
    abc.NextBookedPatientId,
    (abc.NextBookedPatientId - abc.PreviouslyBookedPatientId) / 2 AS Gap
FROM   (SELECT ps.PatientId,
               (SELECT MAX(PatientId)
                FROM   PatientStay
                WHERE  PatientId < ps.PatientId) AS PreviouslyBookedPatientId,
               (SELECT MIN(PatientId)
                FROM   PatientStay
                WHERE  PatientId > ps.PatientId) AS NextBookedPatientId
        FROM   PatientStay AS ps) AS abc
ORDER BY Gap DESC
